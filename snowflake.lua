...
local has_intro = database.read("has_intro_snowflake_tech")

local tab_snowflake_tech=ui.new_combobox(tab, place, "\aaac8fbff SnowFlake \affffffffTech", {"AA","AA-Extra","Visuals","Misc","Config"})
...
local aa = {
    aa_states = {"STANDING","RUNING","IN AIR","AIR+C","CROUCH","C+MOVE","SLOW","MANUAL","FAKELAG","LEGIT"},
    MANUAL_state = "reset",
...
local new_menu_references = {
    ...
    ["visuals"] = {
        ...
        enable_indicators = ui.new_combobox(tab,place,"indicators",{"off","default", "minimal"}),
        indicator_color = ui.new_color_picker(tab, place, "indicatorz", 101, 130, 190, 255),
        snowflake_label = ui.new_checkbox(tab,place,"snowflake"),
        snowflake_color = ui.new_color_picker(tab, place, "snowflake", 255, 255, 255, 150),
        ...
        extra_visuals = ui.new_multiselect(tab, place, "> extras",{"beta logs", "at target flag"}),
    ...
    ["misc"] = {
        ...
        enable_kill_say = ui.new_combobox(tab, place, "kill say",{"Disabled","SnowFlake tech"}),
        ...
        fastLadder = ui.new_checkbox(tab, place, "\aaac8fbffFast \affffffffladder"),
    },
...
local misc_functions = {
    kill_say = {["SnowFlake tech"] = {"SnowFlake Tech By Jiu"}},
    clan_tag = {"S","Sn","Sno","Snow","SnowF","SnowFl","SnowFla","SnowFlake","SnowFlake","SnowFlake.T","SnowFlake.Te","SnowFlake.Tec","SnowFlake.Tech",},
    ct_tick = 0,
    ct_old_tick = 0,
    save_tick = 0,
...
for k,v in ipairs(aa.aa_states) do
    ...
    if v ~= "MANUAL" and v ~= "LEGIT" then
        ...
        new_menu_references["aa"].aa_state[v].tickmode = ui.new_combobox(tab,place,light_blue .. new_streng .. white .."tick mode\n" .. v,{"static", "random", "jitter"})
        new_menu_references["aa"].aa_state[v].ticker_slider = ui.new_slider(tab,place, light_blue .. new_streng .. white .."min ticks \n" .. v, 1, 15, 1)
        new_menu_references["aa"].aa_state[v].ticker_slider1 = ui.new_slider(tab,place, light_blue .. new_streng .. white .."max ticks \n" .. v, 1, 15, 1)
    end
...
function visual_functions:defensive_indicator()
    ...
    if ui.get(new_menu_references["visuals"].enable_defensive_indicator) then
        ...
        if visual_functions.defensive_ready == true then
            ...
            renderer.text(w / 2 , visual_functions.is_defensive and h / 2  * 0.5 - 20 or h / 2  * 0.5 - 10 ,255,255,255,lerp_alpha,"c",0,visual_functions.is_defensive and "SNOWFLAKE Tech-Exploit" or "- SNOWFLAKE Tech-Exploit -")
...
function visual_functions:slow_indicator()
    ...
    if NOVINHA_QUEISSO < 100 then
        ...
        renderer.text(sizer.x /2,sizer.y/2- 240,230,230,230,255,"c",nil,string.format("SNOWFLAKE Tech charging %s",math.floor(100 - NOVINHA_QUEISSO)) .. "%")
...
function aa_functions:setup(cmd)
    ...
    elseif on_ground and cmd.in_duck == 1 and get_velocity(lp) < 3 then
        aa_functions.aa_number = "CROUCH"
    elseif on_ground and cmd.in_duck == 1 and get_velocity(lp) >= 3 then
        aa_functions.aa_number = "C+MOVE"
    elseif not on_ground and cmd.in_duck == 1 then
        aa_functions.aa_number = "AIR+C"
    ...
    if aa_functions.aa_number ~= "LEGIT" and aa_functions.aa_number ~= "MANUAL" then
        local tick = ui.get(new_menu_references["aa"].aa_state[aa_functions.aa_number].ticker_slider)
        local tick1 = ui.get(new_menu_references["aa"].aa_state[aa_functions.aa_number].ticker_slider1)
        local tickmode = ui.get(new_menu_references["aa"].aa_state[aa_functions.aa_number].tickmode)
        local delaytick = 0
        local myArray = {tick, tick1}
        if tickmode == "static" then
            delaytick = tick
        elseif tickmode == "random" then
            delaytick = math.random(tick, tick1)
        elseif tickmode == "jitter" then
            delaytick = math.random(myArray[1], myArray[2])
        end

        if globals.tickcount() > current_tickcount + delaytick then
            if cmd.chokedcommands == 0 then
...
local function text_plus(update_tick,text,speed)
    ...
    text_render = string.sub(text,0,string_minus)
...
function visual_functions:indicators()
    ...
    local offset_x = scoped and  5.5 or 0
    local J1u = scoped and  17.5 or 0
    ...
    if last_state ~= aa_functions.aa_number then
        if #last_state < #aa_functions.aa_number then
            ...
            text_index = #aa_functions.aa_number 
        end
    end
    ...
    text_index,text_string = text_plus(text_index,aa_functions.aa_number,0)
   


     
    renderer.text(w / 2 + math.ceil(text_anim[2]) + J1u,h / 2 + 34,255, 255, 255, 255,"c-",0, text_string )
    

    renderer.text(w / 2 + math.ceil(text_anim[1])-1,h / 2 + 24,indicator_color[1],indicator_color[2],indicator_color[3],indicator_color[4],"c-",0,"SNOW FLAKE.TECH")
    
   
    local dt = ""
    if ui.get(menu_reference.dt[2]) and ui.get(menu_reference.os[2]) then 
        dt = "NO TP"
    else
        dt = "DT"
    end
    ...
    if ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) then 
        -- 渲染DT状态文本，应用X轴偏移量
        renderer.text(w / 2 + (text_anim[2] - 1) + offset_x, h / 2 + 34 + math.ceil(add_y[1]), 255, 255, 255, anti_aim_funcs.get_double_tap() and 255 or 130, "c-", 0, dt)
    end

    if ui.get(menu_reference.os[1]) and ui.get(menu_reference.os[2]) and not ui.get(menu_reference.dt[2]) then
        -- 渲染HS状态文本，应用X轴偏移量
        renderer.text(w / 2 + math.ceil(text_anim[3]) - 1 + offset_x, h / 2 + 34 + math.ceil(add_y[2]), 255, 255, 255, 255, "c-", 0, "HS")
    end
    ...
    renderer.text(w / 2 - 2 + math.ceil(text_anim[4]) ,h / 2 + 44 + math.ceil(addition),255,255,255,ui.get(menu_reference.freestand[1]) and 255 or 130,"c-",0,"FS")
    renderer.text(w / 2 - 18 + math.ceil(text_anim[4]),h / 2 + 44 + math.ceil(addition),255,255,255,ui.get(menu_reference.forcebaim) and 255 or 130,"c-",0,"BAIM")
    renderer.text(w / 2 + 16 +  math.ceil(text_anim[4]),h / 2 + 44 + math.ceil(addition),255,255,255,ui.get(menu_reference.quickpeek[2]) and ui.get(menu_reference.quickpeek[1]) and 255 or 130,"c-",0,"PEEK")
    local text = {"SnowFlake.Tech",dt,"HS","BAIM"}
    for i = 1,#text do
        ...
        if i == 4 then
            ...
            text_anim[i] = lerp(text_anim[i],can2 and measure.x/2 - 3 or 0,globals.frametime() * 15)
        end
...
function visual_functions:indicators2()
    ...
    renderer.text(w / 2 + math.ceil(text_anim[1]) + J2u,h / 2 + 24,indicator_color[1],indicator_color[2],indicator_color[3],indicator_color[4],"cb",0,"Snowflake.Tech")
    ...
    if ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) then 
        renderer.text(w / 2 + math.ceil(text_anim[3]),h / 2 + 33 + math.ceil(add_y[1]),255,255,255,anti_aim_funcs.get_double_tap() and 255 or 130 , "-c", 0, double)
    end

    if ui.get(menu_reference.os[1]) and ui.get(menu_reference.os[2]) and not ui.get(menu_reference.dt[2]) then
        renderer.text(w / 2 + math.ceil(text_anim[4]),h / 2 + 33+ math.ceil(add_y[2]),255,255,255,255 , "-c", 0, "HIDE SHOT")
    end
    ...
    renderer.text(w / 2 + math.ceil(text_anim[5]) ,h / 2 + 43 + math.ceil(addition),255,255,255,ui.get(menu_reference.freestand[1]) and 255 or 130,"-c",0,"FS")
    renderer.text(w / 2 - 18 + math.ceil(text_anim[5]),h / 2 + 43 + math.ceil(addition),255,255,255,ui.get(menu_reference.forcebaim) and 255 or 130,"-c",0,"BAIM")
    renderer.text(w / 2 + 18 +  math.ceil(text_anim[5]),h / 2 + 43 + math.ceil(addition),255,255,255,ui.get(menu_reference.quickpeek[2]) and ui.get(menu_reference.quickpeek[1]) and 255 or 130,"-c",0,"PEEK")

    local text = {"SNOWFLAKE Tech","- " .. text_string .. " -",double,"HIDE SHOT","BAIM"}
    for i = 1,#text do
        local measure = vector(renderer.measure_text("c", text[i]))
        local can2 = scoped_and_box
...
local function draw_snowflake(x, y, radius, color)

    if ui.get(new_menu_references["visuals"].snowflake_label) then
     -- 定义雪花的分支数量和细节
     local branches = 8  -- 增加分支数量
     local branch_angle = 360 / branches
     local branch_length = radius / 3  -- 缩小尺寸
     local detail_length = radius / 6  -- 缩小副分支长度
     
     -- 绘制雪花的分支
     for i = 0, branches - 1 do
         local angle = i * branch_angle
         -- 计算主分支的端点
         local end_x = x + branch_length * math.cos(math.rad(angle))
         local end_y = y + branch_length * math.sin(math.rad(angle))
          
         -- 绘制主分支
         renderer.line(x, y, end_x, end_y, color[1], color[2], color[3], color[4])
          
         -- 绘制主分支的副分支
         local detail_count = 4  -- 每个主分支的副分支数量
         for j = 1, detail_count do
             local offset_angle = 30 * j
             local detail_x = end_x + detail_length * math.cos(math.rad(angle + offset_angle))
             local detail_y = end_y + detail_length * math.sin(math.rad(angle + offset_angle))
               
             -- 绘制副分支
             renderer.line(end_x, end_y, detail_x, detail_y, color[1], color[2], color[3], color[4])
            end
        end
    end
end
...
local center_y = screen_height / 2 + 25 
-- paint 回调函数，用于在屏幕上绘制内容
client.set_event_callback("paint", function()
   local scoped = entity.get_prop(entity.get_local_player(),"m_bIsScoped") == 1 and true or false 
   local Jiu = scoped and  25 or 0
   local SnowFlakecolor =  { ui.get(new_menu_references["visuals"].snowflake_color) } 
   draw_snowflake(center_x - Jiu, center_y - Jiu , 25, {SnowFlakecolor[1],SnowFlakecolor[2],SnowFlakecolor[3],SnowFlakecolor[4]})  -- 白色雪花，尺寸更小
end)   
    ...
    if ui.get(new_menu_references["visuals"].enable_min_dmg_ovrrd) then
        ...
        if ui.get(menu_reference.min_dmg_override[1]) and ui.get(menu_reference.min_dmg_override[2]) then
            ...
            elseif ui.get(menu_reference.min_dmg_override[3]) > 100 then
                visual_functions.damage_indi = "10" .. ui.get(menu_reference.min_dmg_override[3]) - 100
            elseif ui.get(menu_reference.min_dmg_override[3]) <= 100 then
        ...
        elseif  not ui.get(menu_reference.min_dmg_override[1]) or not ui.get(menu_reference.min_dmg_override[2]) then 
            ...
            elseif ui.get(menu_reference.minimum_damage) > 100 then
                visual_functions.damage_indi = "10" .. ui.get(menu_reference.minimum_damage) - 100
            elseif ui.get(menu_reference.minimum_damage) <= 100 then
                visual_functions.damage_indi = ui.get(menu_reference.minimum_damage)
            end
        end
    
        renderer.text(w / 2 + 13, h / 2 - 5 , 255, 255, 255, 255,"c",0,tostring(visual_functions.damage_indi):upper())
    end
end
...
function visual_functions:watermark()
    ...
    text_fade_animation(w - w + 70,h / 2,5,{r=255,g=255,b=255,a=255},{r=wm[1],g=wm[2],b=wm[3],a=wm[4]},"SNOWFLAKE.TECH")
end
...
local function new_menu_visibility()
   ...
   -- ui.set_visible(new_menu_references["aa"].aa_label,ui_menu.selected_tab == 1)
    ui.set_visible(new_menu_references["aa"].aa_state_menu,ui.get(tab_snowflake_tech) == "AA" or ui.get(tab_snowflake_tech) == "AA-Extra" and jitter_page)
    ui.set_visible(new_menu_references.aa_tab,ui.get(tab_snowflake_tech) == "AA-Extra")
    
    --local anti_bruteforce = ui.get(new_menu_references["aa"]_tab) == "bruteforce"
    for k,v in ipairs(aa.aa_states) do
        ui.set_visible(new_menu_references["aa"].aa_state[v].enabled,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v)
        ui.set_visible( new_menu_references["aa"].aa_state[v].pitch,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v)
        ui.set_visible( new_menu_references["aa"].aa_state[v].pitch_slider,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].pitch) == "custom")
        ui.set_visible( new_menu_references["aa"].aa_state[v].yawbase,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and not MANUAL_page) 
        ui.set_visible( new_menu_references["aa"].aa_state[v].yaw,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and v ~= "LEGIT" and v ~= "MANUAL") 
        ui.set_visible( new_menu_references["aa"].aa_state[v].yawadd,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and not MANUAL_page and ui.get(new_menu_references["aa"].aa_state[v].yaw) ~= "off") 
        ui.set_visible( new_menu_references["aa"].aa_state[v].yawjitter,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v)  
        ui.set_visible( new_menu_references["aa"].aa_state[v].yawjitteradd,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) ~= "off" and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) ~= "jitter ways" and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) ~= "sway")  
        ui.set_visible( new_menu_references["aa"].aa_state[v].yawjitteradd2,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) == "l & r")  
        ui.set_visible( new_menu_references["aa"].aa_state[v].gs_bodyyaw,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v) 
        ui.set_visible( new_menu_references["aa"].aa_state[v].delayed_method,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].gs_bodyyaw) == "delayed yaw") 
        ui.set_visible( new_menu_references["aa"].aa_state[v].gs_bodyyawadd,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].gs_bodyyaw) ~= "off" and ui.get(new_menu_references["aa"].aa_state[v].gs_bodyyaw) ~= "anti bruteforce" and ui.get(new_menu_references["aa"].aa_state[v].gs_bodyyaw) ~= "auto jitter" and  ui.get(new_menu_references["aa"].aa_state[v].gs_bodyyaw) ~= "delayed yaw") 
        ui.set_visible( new_menu_references["aa"].aa_state[v].edgeyaw,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and not MANUAL_page) 
        ui.set_visible( new_menu_references["aa"].aa_state[v].roll,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v) 
        ui.set_visible(new_menu_references["aa"].aa_state[v].jway_counter,false)
        ui.set_visible(new_menu_references["aa"].aa_state[v].swaym, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) == "sway")
        ui.set_visible(new_menu_references["aa"].aa_state[v].swaymx, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) == "sway")
        ui.set_visible(new_menu_references["aa"].aa_state[v].swayspeed, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yawjitter) == "sway")

        if  v ~= "LEGIT" and v ~= "MANUAL" then
            ui.set_visible(new_menu_references["aa"].aa_state[v].yawaddleft, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and (ui.get(new_menu_references["aa"].aa_state[v].yaw) == "jittery" or ui.get(new_menu_references["aa"].aa_state[v].yaw) == "delayed" or ui.get(new_menu_references["aa"].aa_state[v].yaw) == "l & r"))
            ui.set_visible(new_menu_references["aa"].aa_state[v].tickmode, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yaw) == "delayed")
            ui.set_visible(new_menu_references["aa"].aa_state[v].ticker_slider, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yaw) == "delayed")
            ui.set_visible(new_menu_references["aa"].aa_state[v].ticker_slider1, ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yaw) == "delayed")
        end
        
        if v ~= "FAKELAG" and v ~= "LEGIT" and v ~= "MANUAL" then
            ui.set_visible(new_menu_references["aa"].aa_state[v].enabled_exploit,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v)
            ui.set_visible( new_menu_references["aa"].aa_state[v].enabled_defensive_force,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["aa"].aa_state[v].pitch_defensive,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit))
            ui.set_visible( new_menu_references["aa"].aa_state[v].pitch_custom_defensive,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].pitch_defensive) == "custom" and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit))
            ui.set_visible( new_menu_references["aa"].aa_state[v].yaw_defensive,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["aa"].aa_state[v].yawadd_defensive,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].yaw_defensive) ~= "off" and ui.get(new_menu_references["aa"].aa_state[v].yaw_defensive) ~= "randomize" and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["aa"].aa_state[v].skitter_defensive,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit)) 
            ui.set_visible(new_menu_references["aa"].aa_state[v].skitter_slider_defensive,ui.get(tab_snowflake_tech) == "AA" and ui.get(new_menu_references["aa"].aa_state_menu) == v and ui.get(new_menu_references["aa"].aa_state[v].skitter_defensive) and ui.get(new_menu_references["aa"].aa_state[v].enabled_exploit))
        end

        ui.set_visible(new_menu_references["aa"].label_jitter_ways,ui.get(tab_snowflake_tech) == "AA-Extra" and jitter_page and ui.get(new_menu_references["aa"].aa_state[ui.get(new_menu_references["aa"].aa_state_menu)].yawjitter) ~= "jitter ways")
        for i = 1, 12 do
            ui.set_visible( new_menu_references["aa"].aa_state[v].pedro_aas2[i]  ,ui.get(tab_snowflake_tech) == "AA-Extra" and ui.get(new_menu_references["aa"].aa_state_menu) == v and i <= ui.get(new_menu_references["aa"].aa_state[v].jway_counter) and jitter_page and ui.get(new_menu_references["aa"].aa_state[ui.get(new_menu_references["aa"].aa_state_menu)].yawjitter) == "jitter ways")
    ...
    ui.set_visible(new_menu_references["aa"].freestanding_disabler, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page)
    ui.set_visible(new_menu_references["aa"].freestanding_options, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page)
    ui.set_visible(jitter_add, ui.get(tab_snowflake_tech) == "AA-Extra" and jitter_page and ui.get(new_menu_references["aa"].aa_state[ui.get(new_menu_references["aa"].aa_state_menu)].yawjitter) == "jitter ways" )
    ui.set_visible(jitter_remove, ui.get(tab_snowflake_tech) == "AA-Extra" and jitter_page and ui.get(new_menu_references["aa"].aa_state[ui.get(new_menu_references["aa"].aa_state_menu)].yawjitter) == "jitter ways")
    ui.set_visible(new_menu_references["keys"].enable_key, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page)
    ui.set_visible(new_menu_references["keys"].MANUAL_forward, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page and keys)
    ui.set_visible(new_menu_references["keys"].MANUAL_left, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page and keys)
    ui.set_visible(new_menu_references["keys"].MANUAL_right, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page and keys)
    ui.set_visible(new_menu_references["keys"].MANUAL_reset, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page and keys)
    ui.set_visible(new_menu_references["keys"].global_fs, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page and keys)
    ui.set_visible(new_menu_references["aa"].anti_kniv, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page)
    ui.set_visible(new_menu_references["aa"].safe_dangerous, ui.get(tab_snowflake_tech) == "AA-Extra" and main_page)
   

    for h = 1, 4 do
       
        ui.set_visible(new_menu_references["bruteforce"][h],ui.get(tab_snowflake_tech) == "AA-Extra" and bruteforce_page)
    end

    --visuals
    --ui.set_visible(new_menu_references["visuals"].visual_label,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].zeus,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].enable_indicators,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].draw_logs,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].indicator_color,ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].enable_indicators) ~= "off")
    ui.set_visible(new_menu_references["visuals"].scoped_animation,ui.get(tab_snowflake_tech) == "Visuals" )
    ui.set_visible(new_menu_references["visuals"].indicator_MANUAL_color,ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].enable_MANUAL) ~= "off")
    ui.set_visible(new_menu_references["visuals"].enable_MANUAL,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].enable_min_dmg_ovrrd,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].enable_desync_indicator,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].enable_velocity_adaptive, ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].enable_MANUAL) ~= "off")
    ui.set_visible(new_menu_references["visuals"].enable_defensive_indicator,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].slow_indicator,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].indicator_desync_color,ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].enable_desync_indicator))
    ui.set_visible(new_menu_references["visuals"].indicator_defensive_color,ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].enable_defensive_indicator))
    ui.set_visible(new_menu_references["visuals"].indicator_slow_color,ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].slow_indicator))
    ui.set_visible(new_menu_references["visuals"].aim_logs_label, ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].aimer_legs))
    ui.set_visible(new_menu_references["visuals"].aim_logs_outline_color, ui.get(tab_snowflake_tech) == "Visuals" and ui.get(new_menu_references["visuals"].aimer_legs))
    ui.set_visible(new_menu_references["visuals"].watermark_color, ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].watermark_label, ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].snowflake_label, ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].snowflake_color, ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].extra_visuals,ui.get(tab_snowflake_tech) == "Visuals")
    ui.set_visible(new_menu_references["visuals"].noti_timer,ui.get(tab_snowflake_tech) == "Visuals" and (ui.get(new_menu_references["visuals"].aimer_legs) or ui.get(new_menu_references["visuals"].whatsappi)))
    ui.set_visible(new_menu_references["visuals"].noti_padding,ui.get(tab_snowflake_tech) == "Visuals" and  ui.get(new_menu_references["visuals"].whatsappi))
    
    --misc
    --ui.set_visible(new_menu_references["misc"].misc_label,ui.get(tab_snowflake_tech) == "Misc")
    ui.set_visible(new_menu_references["misc"].enable_clan_tag,ui.get(tab_snowflake_tech) == "Misc")
    ui.set_visible(new_menu_references["misc"].enable_kill_say,ui.get(tab_snowflake_tech) == "Misc")
    ui.set_visible(new_menu_references["misc"].enable_local_animations,ui.get(tab_snowflake_tech) == "Misc")
    ui.set_visible(new_menu_references["misc"].fastLadder,ui.get(tab_snowflake_tech) == "Misc")
    --config
    --ui.set_visible(new_menu_references.config.config_label,ui.get(tab_snowflake_tech) == "Config")
    ui.set_visible(new_menu_references.config.export_file,ui.get(tab_snowflake_tech) == "Config")
    ui.set_visible(new_menu_references.config.import_config,ui.get(tab_snowflake_tech) == "Config")
end
...
function visual_functions:whatsapp_notifications()
    ...
    for i=1,#aim_logs do
        ...
        renderer.text(w/ 2 - measure[1]/2 + 30,math.ceil(aim_logs[i].pos_animation) - 32 + not_x,220,220,220,255,"",nil, aim_logs[i].state == true and "SNOWFLAKE Tech ~ \a96C83CFFHit\aFFFFFFFF" or "SNOWFLAKE Tech ~ \aFF0032FFMiss\aFFFFFFFF")
...
function visual_functions:NIGGASSSSSS()
    ...
    renderer.text(w/ 2 - measure[1]/2 + 35,lerp_alpha3 + 8 - 10,220,220,220,255,"",nil,"SNOWFLAKE Tech ~ " .. string.format("%s",is_on == true and "\a96C83CFFNotification\aFFFFFFFF" or "\aFF0032FFNotification\aFFFFFFFF"))
    

    renderer.text(w/ 2 - measure[1]/2 + 35,lerp_alpha3 + 20 - 10,220,220,220,255,"",nil, "Hit \a96C83CFFBhop\aFFFFFFFF in the \a96C83CFFhead\aFFFFFFFF for \a96C83CFF99\aFFFFFFFF (\a96C83CFF0\aFFFFFFFF health remaining)")
...
function visual_functions:welcome_to_snowflake_tech()
   local w,h = client.screen_size()
    intro[#intro + 1] ={
        message = "Welcome to , ",
...
visual_functions.welcome_to_snowflake_tech()
...
function misc_functions:clantag()

    local clantag_prev = nil
    if ui.get(new_menu_references["misc"].enable_clan_tag) then
            local cur = math.floor(globals.tickcount() / 20) % #misc_functions.clan_tag
            local clantag = misc_functions.clan_tag[cur+1]
...
function aa_functions:fastLadder(cmd)

    if ui.get(new_menu_references["misc"].fastLadder) then
...
client.set_event_callback("net_update_end", misc_functions.clantag)

client.set_event_callback("setup_command",function(cmd)
    aa_functions:setup(cmd)
    aa_functions:fastLadder(cmd)
...
client.set_event_callback("shutdown", function()
    ...
    database.write("has_intro_snowflake_tech", "has")
